...
static void cs_cmd_mass_deprotect(sourceinfo_t *si, int parc, char *parv[]);
...
command_t cs_mass_deprotect = { "DEPROTECT", N_("De-Protects all channel users on a channel."),
	AC_NONE, 2, cs_cmd_mass_deprotect, { .path = "cservice/mass_deprotect" } };
...
void _modinit(module_t *m)
{
	...
	command_add(&cs_mass_deprotect, *cs_mass_cmds);
}

void _moddeinit(module_unload_intent_t intent)
{
	command_delete(&cs_mass_protect, *cs_mass_cmds);
	command_delete(&cs_mass_deprotect, *cs_mass_cmds);
}
...
static void cs_cmd_mass_deprotect(sourceinfo_t *si, int parc, char *parv[])
{
	...
	logcommand(si, CMDLOG_DO, "MASS:DEPROTECT: \2%s\2", mc->name);

	command_success_nodata(si, _("Mass De-Protected channel users in \2%s\2."), channel);
}
