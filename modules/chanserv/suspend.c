	...
	if (!mt)
	{
		...
		if (ca != NULL)
		{
			if (ca->level & CA_SUSPENDED)
				command_fail(si, fault_nochange, _("\2%s\2 is already on the SUSPEND list for \2%s\2"), uname, mc->name);
			return;
		}
		...
		if (ca != NULL)
		{
			command_fail(si, fault_nochange, _("The more general mask \2%s\2 is already on the SUSPEND list for \2%s\2"), ca->host, mc->name);
			return;
		}

		if (ca == NULL)
		{
			command_fail(si, fault_nochange, _("No matching nick or hostmask with flags on \2%s\2"), mc->name);
			return;
		}
		...
		if (duration > 0)
		{
			...
			logcommand(si, CMDLOG_SET, "SUSPEND:ADD: \2%s\2 on \2%s\2, expires in %s.", uname, mc->name,timediff(duration));
			command_success_nodata(si, _("SUSPENSION on \2%s\2 was successfully added for \2%s\2 and will expire in %s."), uname, mc->name,timediff(duration) );

			timeout = suspend_add_timeout(mc, NULL, uname, expireson);
		...
		else
		{
			...
			command_success_nodata(si, _("SUSPENSION on \2%s\2 was successfully added to the SUSPEND list for \2%s\2."), uname, mc->name);
		}
